...
def enthalpy(T):
    ...
    return  np.array([t,  t**2 / 2.0, t**3 / 3.0, t**4 / 4.0, -1.0 / t, 1.0, 0.0, -1.0])
...
def entropy(T):
    ...
    return np.array([np.log(t), t,  t**2 / 2.0,  t**3 / 3.0, -1.0 / (2.0 * t**2), 0.0, 1.0, 0.0])
...
def specific_heat(T):
    ...
    return np.array([1,  t, t**2, t**3, 1.0/t])
...
def enthalpy_rxn_kj_mol(rxn, T):
    ...
    return rxn.HF298.dot(rxn.v) + (rxn.coeffs.dot(enthalpy(T))).dot(rxn.v)
...
def entropy_rxn_kj_mol(rxn, T):  
    ...
    return (rxn.coeffs.dot(entropy(T)/1000.0)).dot(rxn.v)
...
def gibbs_rxn_kj_mol(rxn, T):  
    ...
    return enthalpy_rxn_kj_mol(rxn, T) - T*entropy_rxn_kj_mol(rxn, T)
...
def cpi_j_mol_K(species, T):  
    ...
    return species.coeffs.dot(specific_heat(T))
...
def cp_mix_j_mol_K(species, T):
    ...
    return species.coeffs.dot(specific_heat(T)).dot(species.y)
...
def ci_mol_m3(species, T, P):
    ...
    return species.y*P/(8.314e-05*T)
...
def c_total_mol_m3(species, T, P):
    ...
    return np.sum(ci_mol_m3(species, T, P))
...
def mr_mix_kg_mol(species):
    ...
    return species.Mr.dot(species.y)